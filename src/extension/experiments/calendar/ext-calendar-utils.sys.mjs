...
export function convertItem(item, options, extension) {
  ...
  if (!props.calendarId) {
    throw new ExtensionError(`Calendar item ${item.id} has no calendar id`);
  }

  // Keep instance as an iCal string so return values stay schema-safe and
  // structured-cloneable (jCal date-time values can be arrays).
  const recId = item.recurrenceId?.getInTimezone(cal.timezoneService.UTC)?.icalString;
  if (recId) {
    props.instance = recId;
  }
